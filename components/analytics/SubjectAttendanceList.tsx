...
/**
 * Subject-wise Attendance Breakdown Component
 *
 * Lists all registered courses, attended vs. total lectures, percentage progress bar,
 * eligibility badges, and warning notes calculating needed classes for low attendance.
 */

import React from "react";
import type { SubjectAttendance } from "@/lib/types/analytics";
import { calculateNeededClasses } from "@/lib/utils/calculations";
...
/**
 * Renders detailed course-by-course attendance compliance and shortage warnings.
 *
 * @param props - Component configuration containing subject attendance records.
 * @returns Subject attendance list container element.
 */
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        {attendanceData.map((item) => {
          ...
          const neededClasses = isWarning
            ? calculateNeededClasses(item.attended, item.total, 0.75)
            : 0;
          ...
          return (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2.5">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  >
                    ...
                    <span>{isWarning ? "Shortage Warning" : "Eligible"}</span>
                  </span>
                </div>
              </div>
              ...
              {/* Warning Alert Note */}
              {isWarning && (
                <div className="mt-3 flex items-center gap-2 p-3 rounded-xl bg-rose-100/60 dark:bg-rose-950/50 text-rose-800 dark:text-rose-200 text-xs border border-rose-200/80 dark:border-rose-900/60">
                  <svg
                    className="w-4 h-4 shrink-0 text-rose-600 dark:text-rose-400"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
